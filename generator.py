...
def get_training_and_validation_generators(data_file, batch_size=4, data_split=0.8, validation_keys_file="val_keys.pkl", training_keys_file="train_keys.pkl", slice_based=True, validation_batch_size=4):
    ...
    validation_generator = data_generator(data_file, validation_list,
                                        batch_size=batch_size,
                                        slice_based=slice_based)
    num_training_steps = get_number_of_steps(get_number_of_instances(data_file, training_list, slice_based), batch_size)
    num_validation_steps = get_number_of_steps(get_number_of_instances(data_file, validation_list, slice_based), batch_size)
    return [training_generator, validation_generator, num_training_steps, num_validation_steps]                                        

def data_generator(data_file, index_list, batch_size, slice_based=True):
    ...
    while True:
        ...
        while len(index_list) > 0:
            ...
            add_data(x_list, y_list, data_file, index, slice_based)
            if len(x_list) == batch_size or (len(index_list) == 0 and len(x_list) > 0):
                yield np.asarray(x_list), np.asarray(y_list)
                x_list = list()
                y_list = list()

def get_number_of_instances(data_file, index_list, slice_based=None, skip_blank=True):
    if slice_based:
        z = data_file.root.data.shape[-1]
